...
- **Wiring the remaining 6 prototype modules** (Timetable & Schedules, Live Classes, Assignments, Feedback, Jobs, Activities) — Curriculum, Batches, and Lesson Plans are now wired (see §9). Planned as incremental follow-up work, per the client's "editable at every step" direction. Do this one module at a time, not all at once — Timetable is the natural next pick since Live Classes depends on it.
