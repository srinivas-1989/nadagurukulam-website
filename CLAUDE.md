...
- **Wiring the remaining 5 prototype modules** (Live Classes, Assignments, Feedback, Jobs, Activities) — Curriculum, Batches, Lesson Plans, and Timetable are now wired (see §9). Planned as incremental follow-up work, per the client's "editable at every step" direction. Do this one module at a time, not all at once — Live Classes is the natural next pick since it reads timetable slots for its room links.
