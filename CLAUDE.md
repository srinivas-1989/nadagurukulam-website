...
| 3.5 | Live clickable prototype (added mid-process, see §9) | **Done** (all 13 modules wired) |
...
3. **Curriculum** — discipline structure + syllabus; **hybrid record** (see §6) — structured fields in Postgres, syllabus body in MongoDB. Supports both **Masters (MPA)** and **Undergraduate** course types with dynamic semester selection (4 semesters for Masters, 6 for Undergraduate).
...
- **Wired (functional) modules:** `overview` (stat tiles + Super-Admin-only inline CMS editor), `users` (list/add/toggle-active), `events` (list/draft/publish-toggle/delete — draft allowed for super_admin/admin/staff, publish/unpublish restricted to super_admin/admin), `enquiries` (list/status-dropdown/simulate-submission-form), `curriculum` (list/add/delete disciplines with **course type** (Masters/Undergraduate) and dynamic semesters — Super Admin/Admin manage, other roles read-only with an honest note that per-user "own discipline" scoping needs real auth), `batches` (list/add/delete, with discipline and faculty dropdowns sourced live from Curriculum and Users), `lessonplans` (a second workflow pattern distinct from Events' draft→publish: **draft → submitted → approved / needs_revision**, with Teacher/Guest Faculty drafting+resubmitting and Admin/Super Admin approving or returning with a reviewer note), `timetable` (recurring day/time/room slots per batch, Super Admin/Admin manage; **conflict detection** via `slotsOverlap()` — a new slot that overlaps an existing one for the same batch or the same room prompts a `confirm()` before it's added; a `console.assert` self-check on the overlap function runs at boot), `liveclasses` (Jitsi room scheduling, auto attendance on join/leave, join/cancel buttons, room links generated from timetable slots), `assignments` (batch selection, type audio/video/file/text, status open/closed/graded inline toggle, Delete for super_admin/admin), `feedback` (batch selection, type faculty→student/student→faculty/peer, comment table, author/comment/date columns), `jobs` (staff draft → Admin publish/close with status badges), `activities` (batch selection, category Performance/Competition/Workshop/Achievement, date + description table). Each module has its own add form and data table with role-gated Delete actions per the §3 permission matrix.

- **Seeded real data:** 3 events, 2 enquiries, 6 users (one per role), 6 disciplines (the actual six taught), 2 batches, 2 lesson plans (one approved, one submitted-awaiting-review), 3 timetable slots, 1 `cms/home` doc — all written via `write_db`/`batch`, not page JS.

The prototype now covers all 13 modules per the Phase 3.5 roadmap. **No placeholder modules remain** — every module from Overview through Activities is functional with its own form and table, editable inline within the permission bounds of the role picker.
...
- **All 13 prototype modules are now wired** (see §9) — Live Classes, Assignments, Feedback, Jobs, and Activities were completed incrementally per the client's "editable at every step" direction. No placeholder modules remain.
