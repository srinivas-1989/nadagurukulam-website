...
export default function Home() {
  ...
  const handleAddDiscipline = async (e) => {
    ...
    if (cat?.duration_value) {
      ...
      let progYears;
      if (newDiscStructure==='monthly') progYears = Number(newDiscMonthCount)/12;
      else if (newDiscStructure==='semester') { const tot=Number(newDiscTotalSems)||4, per=Number(newDiscSemPerYear)||2; progYears = per ? tot/per : Number(newDiscYearCount); }
      else progYears = Number(newDiscYearCount);
    ...
    else if (newDiscStructure==='semester') { const tot=Number(newDiscTotalSems)||4, per=Number(newDiscSemPerYear)||2; if(!Number.isFinite(tot)||!Number.isFinite(per)||tot<1||per<1||per>4){ alert('Enter valid Total and Semesters per Year (1-4).'); return; } if(tot%per!==0){ alert('Total semesters must be divisible by semesters per year.'); return; } const yrs=tot/per; if(!Number.isFinite(yrs)||yrs<1||yrs>10){ alert('Derived years must be 1-10.'); return; } body.year_count=yrs; body.semesters_per_year=per; }
                      ...
                      {newDiscStructure === 'semester' && (
                        <>
                          <label style={{ display: 'flex', flexDirection: 'column', fontSize: '11.5px', fontWeight: 600, color: 'var(--text-soft)', gap: '3px' }}>Total Semesters {newDiscCatLimit ? <span style={{ fontWeight:400, color:'var(--text-faint)' }}>(max {newDiscMaxYears*4})</span> : null}
                            <input type="number" min="1" max={newDiscMaxYears*4} value={newDiscTotalSems} onChange={e => setNewDiscTotalSems(e.target.value)} style={{ padding: '8px', border: '1px solid var(--border)', borderRadius: '4px', width: '110px' }} />
                          </label>
                          ...
                          <label style={{ display: 'flex', flexDirection: 'column', fontSize: '11.5px', fontWeight: 600, color: 'var(--text-soft)', gap: '3px' }}>Years (derived) {newDiscCatLimit ? <span style={{ fontWeight:400, color:'var(--text-faint)' }}>(max {newDiscMaxYears})</span> : null}
                            <input type="number" value={Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?String(Number(newDiscTotalSems)/Number(newDiscSemPerYear||1)):''} readOnly placeholder="—" title={Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'Auto: Total ÷ Per Year':'Total must divide equally by Per Year'} style={{ padding: '8px', border: `1px solid ${Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'var(--border)':'var(--primary)'}`, borderRadius: '4px', width: '90px', background: 'var(--bg)' }} />
                          </label>
                          <span style={{ fontSize: '11px', color: Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'var(--text-faint)':'var(--primary)', alignSelf: 'flex-end', paddingBottom: '8px' }}>{Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0 ? `${Number(newDiscTotalSems)/Number(newDiscSemPerYear||1)}y × ${Number(newDiscSemPerYear)}/yr = ${Number(newDiscTotalSems)} sem` : 'Total must divide equally'}</span>
                        </>
                      )}
                        ...
                        return editingDisc === d.id ? (
                          <tr key={d.id} style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-saffron)' }}>
                            ...
                            <td style={{ padding: '8px' }}>
                              ...
                              {discEditStructure === 'semester' && <><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Total<br /><input type="number" min="1" max={editDiscMaxYears*4} value={discEditTotalSems} onChange={e => setDiscEditTotalSems(e.target.value)} placeholder="Total" style={{ padding: '6px', border: '1px solid var(--border)', borderRadius: '4px', width:'100%', marginBottom:'4px' }} /></label><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Per Year<br /><input type="number" min="1" max="4" value={discEditSemPerYear} onChange={e => setDiscEditSemPerYear(e.target.value)} placeholder="Per Year" style={{ padding: '6px', border: '1px solid var(--border)', borderRadius: '4px', width: '100%', marginBottom:'4px' }} /></label><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Years (derived){editDiscCatLimit?<span style={{color:'var(--text-faint)'}}> (max {editDiscMaxYears})</span>:null}<br /><input value={Number(discEditTotalSems)%Number(discEditSemPerYear||1)===0?String(Number(discEditTotalSems)/Number(discEditSemPerYear||1)):''} readOnly placeholder="—" title={Number(discEditTotalSems)%Number(discEditSemPerYear||1)===0?'Auto: Total ÷ Per Year':'Total must divide equally by Per Year'} style={{ padding: '6px', border: `1px solid ${Number(discEditTotalSems)%Number(discEditSemPerYear||1)===0?'var(--border)':'var(--primary)'}`, borderRadius: '4px', width: '100%', marginBottom:'4px', background:'var(--bg)' }} /></label></>}
                            </td>
                            <td style={{ padding: '8px' }}>
