        ...
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', flex: 1, minHeight: '75vh' }}>
          ...
          <main style={{ padding: '36px', maxWidth: '940px', width: '100%' }}>
            ...
            {activeModule === 'feedback' && (
              <div>
                {(role === 'super_admin' || role === 'admin' || role === 'teacher' || role === 'guest_faculty') && (
                  <form onSubmit={handleAddFeedback} style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '16px', borderRadius: '8px', marginBottom: '24px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            {activeModule === 'activities' && (
              <div>
                {(role === 'super_admin' || role === 'admin' || role === 'teacher' || role === 'guest_faculty' || role === 'staff') && (
                  <form onSubmit={handleAddActivity} style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '16px', borderRadius: '8px', marginBottom: '24px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    ...
                    <button type="submit" style={{ background: 'var(--primary)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer', flex: '1 1 100%' }}>{role === 'staff' ? 'Add Activity' : 'Submit Activity'}</button>
                  </form>
                )}
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
                        ...
                        {dbData.activities.map(a => {
                          ...
                          return (
                            <tr key={a.id} style={{ borderBottom: '1px solid var(--border)' }}>
                              ...
                              <td style={{ padding: '12px' }}>
                                {(role === 'super_admin' || role === 'admin' || role === 'staff') && <button onClick={() => handleDelete('activities', a.id)} style={{ background: 'none', border: '1px solid var(--primary)', color: 'var(--primary)', padding: '2px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>Delete</button>}
                              </td>
                            </tr>
                          );
                        })}
            ...
            {![ 'overview', 'users', 'curriculum', 'liveclasses', 'assignments', 'feedback', 'jobs', 'activities'].includes(activeModule) && (
