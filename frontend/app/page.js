...
export default function Home() {
  ...
  const handleAddDiscipline = async (e) => {
    ...
    else if (newDiscStructure==='semester') { const tot=Number(newDiscTotalSems)||4; const per=Number(newDiscSemPerYear)||2; if(tot%per!==0){ alert('Total semesters must divide equally by semesters per year.'); return; } const yrs=tot/per; body.year_count=yrs; body.semesters_per_year=per; }
                  ...
                  <form onSubmit={handleAddDiscipline} style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '16px', borderRadius: 'var(--radius-xl)', display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                      ...
                      {newDiscStructure === 'semester' && (
                        <>
                          <label style={{ display: 'flex', flexDirection: 'column', fontSize: '11.5px', fontWeight: 600, color: 'var(--text-soft)', gap: '3px' }}>Total Semesters {newDiscCatLimit ? <span style={{ fontWeight:400, color:'var(--text-faint)' }}>(max {newDiscMaxYears*4})</span> : null}
                            <input type="number" min="1" max={newDiscMaxYears*4} value={newDiscTotalSems} onChange={e => { const v=Number(e.target.value)||1; setNewDiscTotalSems(e.target.value); const y=Number(newDiscYearCount)||1; if(v%y===0) setNewDiscSemPerYear(String(v/y)); }} style={{ padding: '8px', border: '1px solid var(--border)', borderRadius: '4px', width: '110px' }} />
                          </label>
                          <label style={{ display: 'flex', flexDirection: 'column', fontSize: '11.5px', fontWeight: 600, color: 'var(--text-soft)', gap: '3px' }}>Semesters / Year
                            <input type="number" min="1" max="4" value={newDiscSemPerYear} onChange={e => { const v=Number(e.target.value)||1; setNewDiscSemPerYear(e.target.value); const y=Number(newDiscYearCount)||1; const tot=Number(newDiscTotalSems)||4; if(tot!==y*v) { if(tot% y===0) { /* keep total */ const ny=Math.ceil(tot/v); if(ny!==y) setNewDiscYearCount(String(ny)); } else setNewDiscTotalSems(String(y*v)); } }} style={{ padding: '8px', border: '1px solid var(--border)', borderRadius: '4px', width: '95px' }} />
                          </label>
                          <label style={{ display: 'flex', flexDirection: 'column', fontSize: '11.5px', fontWeight: 600, color: 'var(--text-soft)', gap: '3px' }}>Years (derived) {newDiscCatLimit ? <span style={{ fontWeight:400, color:'var(--text-faint)' }}>(max {newDiscMaxYears})</span> : null}
                            <input type="number" min="1" max={newDiscMaxYears} value={Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?String(Number(newDiscTotalSems)/Number(newDiscSemPerYear||1)):newDiscYearCount} readOnly={Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0} onChange={e => { setNewDiscYearCount(e.target.value); const y=Number(e.target.value)||1; const sp=Number(newDiscSemPerYear)||1; setNewDiscTotalSems(String(y*sp)); }} style={{ padding: '8px', border: `1px solid ${Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'var(--border)':'var(--primary)'}`, borderRadius: '4px', width: '90px', background: Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'var(--bg)':'#fff' }} title={Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'Auto from Total ÷ Per Year':'Must divide equally'} />
                          </label>
                          <span style={{ fontSize: '11px', color: Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0?'var(--text-faint)':'var(--primary)', alignSelf: 'flex-end', paddingBottom: '8px' }}>{Number(newDiscTotalSems)%Number(newDiscSemPerYear||1)===0 ? `${Number(newDiscTotalSems)/Number(newDiscSemPerYear||1)}y × ${Number(newDiscSemPerYear)}/yr = ${Number(newDiscTotalSems)} sem` : 'must divide equally'}</span>
                        </>
                      )}
                        ...
                        return editingDisc === d.id ? (
                          <tr key={d.id} style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-saffron)' }}>
                            ...
                            <td style={{ padding: '8px' }}>
                              ...
                              {discEditStructure === 'semester' && <><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Total<br /><input type="number" min="1" max={editDiscMaxYears*4} value={discEditSemPerYear ? String(Number(discEditYearCount||1)*Number(discEditSemPerYear||2)) : discEditTotalSems} onChange={e => { const v=Number(e.target.value)||1; setDiscEditTotalSems(e.target.value); const sp=Number(discEditSemPerYear)||1; if(v%sp===0) setDiscEditYearCount(String(v/sp)); }} placeholder="Total" style={{ padding: '6px', border: '1px solid var(--border)', borderRadius: '4px', width:'100%', marginBottom:'4px' }} /></label><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Per Year<br /><input type="number" min="1" max="4" value={discEditSemPerYear} onChange={e => setDiscEditSemPerYear(e.target.value)} placeholder="Per Year" style={{ padding: '6px', border: '1px solid var(--border)', borderRadius: '4px', width: '100%', marginBottom:'4px' }} /></label><label style={{ fontSize: '11px', color: 'var(--text-soft)' }}>Years{editDiscCatLimit?<span style={{color:'var(--text-faint)'}}> (max {editDiscMaxYears})</span>:null}<br /><input type="number" min="1" max={editDiscMaxYears} value={discEditYearCount} onChange={e => { setDiscEditYearCount(e.target.value); const y=Number(e.target.value)||1; const sp=Number(discEditSemPerYear)||1; setDiscEditTotalSems(String(y*sp)); }} placeholder="Years" style={{ padding: '6px', border: '1px solid var(--border)', borderRadius: '4px', width: '100%', marginBottom: '4px', background:'var(--bg)' }} title="Auto from Total ÷ Per Year" /></label></>}
                            </td>
                            <td style={{ padding: '8px' }}>
